...
tresult PLUGIN_API MattVerbController::setComponentState (IBStream* state)
{
    ...
    if(streamer.readFloat(dryWetSaveParam) == false
       || streamer.readFloat(bypassSaveParam) == false
       || streamer.readFloat(modeSaveParam) == false
       || streamer.readFloat(dampingSaveParam) == false
       || streamer.readFloat(widthSaveParam) == false
       || streamer.readFloat(roomSizeSaveParam) == false
       || streamer.readFloat(preDelaySaveParam) == false
       )
    {
        return kResultFalse;
    }
